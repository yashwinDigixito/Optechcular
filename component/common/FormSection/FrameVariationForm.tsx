...
export default function FrameVariationForm({ formik }: Props) {
  ...
  return (
    <FieldArray name="variations">
      {({ push, remove }) => (
        <FormSection
          title="5. Eyewear Frame Color, Size & Price Variations"
          description="Manage eyewear frame color options, sizing specs, and branch retail pricing."
        >
          <Box
            sx={{
              display: "flex",
          ...
          {(formik.values.variations || []).map((v, index) => {
            ...
            const vErrors =
              Array.isArray(errorsArray)
                ? errorsArray[index] as FormikErrors<FrameVariation>
                : undefined;

            const vTouched =
              Array.isArray(touchedArray)
                ? touchedArray[index] as FormikTouched<FrameVariation>
                : undefined;
