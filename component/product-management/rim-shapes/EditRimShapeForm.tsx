...
export default function EditRimShapePage({
  ...
  const shape = React.useMemo(() => {
    return rimShapes.find((item) => item.id === id);
  }, [id]);

  const formik = useFormik({
    initialValues: {
      rimShapeId: shape?.rimShapeId || "",
      rimShapeCode: shape?.rimShapeCode || "",
      shapeName: shape?.shapeName || "",
      shapeCategory: shape?.shapeCategory || "",

      applicableFor: shape?.applicableFor || "",
      categoryName: shape?.categoryName || "",
      brandGroup: shape?.brandGroup || "",
      linkedBrands: shape?.linkedBrands?.join(", ") || "",
      materialUsed: shape?.materialUsed?.join(", ") || "",

      totalProducts: shape?.totalProducts || 0,
      status: shape?.status || "",

      description: shape?.description || "",
      notes: shape?.notes || "",

      createdOn: shape?.createdOn || "",
    ...
    onSubmit: (values) => {
      ...
      console.log("Updated Rim Shape Data Successfully:", payload);
    ...
    <Box sx={{ p: 3, background: "#F8FAFC", minHeight: "100vh" }}>
      ...
      >
        ...
        <Typography sx={{ color: "#64748B", mb: 4, fontSize: "14px" }}>
          Update rim shape details linked with brand, brand group, category, and material information.
        </Typography>
          ...
          <form onSubmit={formik.handleSubmit}>
            <Grid container spacing={1}>
              <FormSection
                ...
                description="Update rim shape identity, category, and brand group information."
              >
                {[
              ...
              <FormSection
                ...
                description="Update linked brands and material types."
              >
              ...
              <FormSection
                ...
                description="Update description and notes for operational reference."
              >
                <Grid size={{ xs: 12 }}>
                  <TextField
                    fullWidth
