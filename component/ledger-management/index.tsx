...
export default function LedgerManagementPage() {
  ...
  const ledgerCount = {
    ...
    active: ledgerData.filter( (ledger) => ledger.status === "Active").length,
    inactive: ledgerData.filter( (ledger) => ledger.status ==="Inactive").length,
  };
  const filteredLedgers =
    ledgerData.filter(
      (ledger) => {
        const matchesSearch = !search || ledger.ledgerName.toLowerCase().includes(search.toLowerCase());
        const matchesStatus = status ? ledger.status === status : true;
        const matchesGroup = ledgerGroup ? ledger.ledgerGroup === ledgerGroup : true;
        return (
          matchesSearch &&
          matchesStatus &&
          matchesGroup
